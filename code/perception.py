...
def color_thresh_rock(img, rgb_thresh=(100, 100, 60)):
    # Create an array of zeros same xy size as img, but single channel
    color_select = np.zeros_like(img[:,:,0])
    # Require that each pixel be above all three threshold values in RGB
    # above_thresh will now contain a boolean array with "True"
...
def perception_step(Rover):
    ...
    threshed_all = color_thresh(warped, (0,0,0))
    threshed_obst = threshed_all - threshed
    ...
    navigable_x_rover, navigable_y_rover = rover_coords(threshed)
    obstacle_x_rover, obstacle_y_rover = rover_coords(threshed_obst)
    ...
    if ((Rover.roll < 0.5 or Rover.roll > 359.5) and (Rover.pitch < 0.02 or Rover.pitch > 359.8)):
